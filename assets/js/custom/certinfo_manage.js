jQuery(document).ready(function() {
	...
	$("#btn_certinfo_delete").on("click", function(){
		if(!checkcertlistchkbox()){
			...
			if(confirm('選択した項目を本当に削除しますか？')){
				...
				$.ajax({
					type: "POST",
					url: base_url+'acertinfo/certdelete',
					data: params,
					dataType: 'text',
					success: function(result){
						...
						if(result == '1'){
							alert("削除が完了しました。");			
							location.href=base_url+"acertinfo/manage";
						}else{
							alert("削除に失敗しました。");
							location.href=base_url+"acertinfo/manage";
						}
	...
	$("#btn_certinfo_update").on("click", function(){
		if(!checkcertlistchkbox()){
			...
			certinfoform.action = base_url + "acertinfo/manage";
			certinfoform.submit();
		}
	});
	///////////
	...
	$.validator.setDefaults({
		submitHandler: function() {
			...
			$.ajax({
				type: "POST",
				url: base_url+'acertinfo/manage_accept',
				data: params,
				dataType: 'text',
				success: function(result){
					if(result == '0'){
