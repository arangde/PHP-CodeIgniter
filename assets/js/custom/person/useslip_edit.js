jQuery(document).ready(function() {
	...
	$("#btn_useslips_delete").on("click", function(){
		if(!checkuseslipslistchkbox()){
			...
			if(confirm('選択した項目を本当に削除しますか？')){
				...
				$.ajax({
					type: "POST",
					url: base_url+'puseslip/usdelete',
					data: params,
					dataType: 'text',
	...
	$("#btn_useslips_update").on("click", function(){
		if(!checkuseslipslistchkbox()){
			...
			useslipfrm.action = base_url + "puseslips/usedit";
			useslipfrm.submit();
		}
	});
	///////////
    ...
    $.validator.setDefaults({
		submitHandler: function() {
			...
			$.ajax({
				type: "POST",
				url: base_url+'puseslip/usedit_accept',
				data: params,
				dataType: 'text',
				success: function(result){
					if(result == '0'){
    ...
    $("#sel_useslip_month").on("change", function(){
    	...
    	useslipfrm.action = base_url + "puseslip/usedit";
    	useslipfrm.submit();
    });
