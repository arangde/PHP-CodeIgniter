...
class User_model extends Base_model
{
	...
	function getUserViewHistory($manager_id){
		$query = "SELECT u.*, uh.view_date FROM users_view_history AS uh 
			LEFT JOIN users as u ON u.user_id=uh.user_viewed_id 
			WHERE uh.manager_id={$manager_id} ORDER BY uh.view_date DESC LIMIT 0, 20";
	...
	/*get user list*/
	public function getUserList($keyword="", $employment=99, $job=99, $office=99, $kana=0, $limit=-1, $start=0){
		...
		if($kana != 0) {
			if($kana == 11) {
				$query .= " AND substr(username, 1, 1) NOT IN (";
			}else{
				$query .= " AND substr(username, 1, 1) IN (";
			}
